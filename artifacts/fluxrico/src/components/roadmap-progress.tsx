          ...
          return (
            <li key={stage.name} className="relative flex items-start gap-3 lg:block" data-testid={`stage-${stage.name.toLowerCase()}`}>
              ...
              <div className="pt-1 lg:mt-3 lg:pt-0">
                <p className={`text-sm font-bold ${state === 'current' ? 'text-[#4E46C0]' : state === 'complete' ? 'text-[#343568]' : 'text-[#888BA8]'}`}><span className="mr-1 text-[0.6rem] font-bold tracking-[0.12em] opacity-70">{stage.number} —</span>{stage.name}</p>
