...
import type { JourneyView } from '@/hooks/use-journey';

type GoalSnapshotProps = {
  goal: string;
...
const STATUS_LABEL: Record<GoalSnapshotProps['status'], string> = {
  exploring: 'Exploring',
...
export function GoalSnapshot({ goal, status, stageIndex, stageTotal, progressPercent, onEdit }: GoalSnapshotProps) {
  return (
    <section className="dashboard-card-lift rounded-[1.65rem] border border-[#DADBF0] bg-[#F1F2FD] p-6 sm:p-7" aria-labelledby="goal-title" data-testid="card-goal-snapshot">
      ...
      <h2 id="goal-title" className="mt-7 max-w-[19rem] text-[1.55rem] font-extrabold leading-[1.08] tracking-[-0.055em] text-[#28295D]">{goal}</h2>
      <div className="mt-8 flex items-end justify-between gap-3">
        <div>
          <p className="text-[0.62rem] font-bold uppercase tracking-[0.16em] text-[#8587A3]">STATUS</p>
          <p className="mt-1 text-xl font-extrabold tracking-[-0.04em] text-[#4F48C5]">{STATUS_LABEL[status]}</p>
        </div>
