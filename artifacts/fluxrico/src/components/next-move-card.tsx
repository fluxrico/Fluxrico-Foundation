...
export function NextMoveCard({ move, stageName, stageNumber, hint, onStart, onViewRoadmap }: NextMoveCardProps) {
  return (
    <section
      className="dashboard-card-lift relative overflow-hidden rounded-[1.65rem] bg-[#211F61] p-6 text-white shadow-[0_15px_35px_rgba(38,34,121,0.16)] sm:p-7"
    ...
    >
      ...
      <h2 id="next-move-title" className="relative mt-8 max-w-[23rem] text-[2.05rem] font-extrabold leading-[1.04] tracking-[-0.065em] text-[#FCFCFF]">
        The next useful step
      </h2>
      <p className="relative mt-3 max-w-[26rem] text-[1.35rem] font-bold leading-[1.15] tracking-[-0.04em] text-white">{move}</p>
      {hint && <p className="relative mt-4 max-w-[26rem] text-sm leading-6 text-[#C2C4E1]">{hint}</p>}
      <button
        type="button"
        onClick={onStart}
        className="fluxrico-focus group relative mt-8 inline-flex min-h-12 items-center gap-4 rounded-full bg-[#F4F3FF] px-5 text-[0.68rem] font-bold uppercase tracking-[0.15em] text-[#302B79] transition-transform hover:-translate-y-0.5 hover:bg-white"
      ...
      {onViewRoadmap && (
        <div className="relative mt-5 flex flex-wrap items-center gap-x-5 gap-y-2">
          ...
          <button
            ...
            className="fluxrico-focus inline-flex items-center gap-1 text-[0.68rem] font-bold uppercase tracking-[0.13em] text-[#A8EAF7] hover:text-white"
